...
LRGASP_calculations <- function (NAME, data.dir, out.dir, platform, functions.dir = "/Users/fpardopalacios/LRGASP/myLRGASP/Evaluation") {
  ...
  setwd(data.dir)
  sq.files <- dir(pattern = "*.txt")
  cat("Evaluation script has being run.\nData used for ", NAME, " pipeline are \n", sq.files, "\n")
  sqanti_data=read.table(sq.files[grep(pattern = "classification", sq.files)], sep = "\t", as.is = T, header = T)
  sqanti_data$submission = NAME
  sqanti_data$Platform = platform
  sqanti_data.junc=read.table(sq.files[grep(pattern = "junctions", sq.files)], sep = "\t", as.is = T, header = T)
